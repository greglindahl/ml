...
export function FacetedSearchWithTypeahead({
  ...
  const getIconForGroup = (groupLabel: string): React.ReactNode => {
    if (groupLabel === "People") {
      return <i className="bi bi-person w-4 h-4 text-muted-foreground inline-flex items-center justify-center leading-none" />;
    }
    return <i className="bi bi-folder w-4 h-4 text-muted-foreground inline-flex items-center justify-center leading-none" />;
  };
      ...
      peopleGroup.facets.filter(f => f.toLowerCase().includes(query) && !selectedFacetValues.includes(f)).slice(0, 3).forEach(facet => {
        ...
        if (count > 0) {
          aiIdentified.push({
            ...
            icon: <i className="bi bi-person w-4 h-4 inline-flex items-center justify-center leading-none" />,
            count,
            category: "People",
            isAiGenerated: true
          });
      ...
      sceneGroup.facets.filter(f => !selectedFacetValues.includes(f)).forEach(facet => {
        ...
        if (count > 0 && (matchesQuery || filteredAssets.some(a => a.tags.some(t => t.toLowerCase() === facet.toLowerCase())))) {
          aiIdentified.push({
            ...
            value: facet,
            label: facet,
            icon: <i className="bi bi-tag w-4 h-4 inline-flex items-center justify-center leading-none" />,
            count,
            category: "Scene",
            isAiGenerated: true
          });
                  ...
                  <div className="flex flex-col gap-1">
                    {recentSearches.map((search, idx) => <div key={`recent-${idx}`} className="flex items-center gap-2 px-2 py-1.5 text-sm rounded hover:bg-accent transition-colors group">
                        <button onClick={() => handleAddSearchTerm(search)} className="flex items-center gap-2 flex-1 text-left">
                          <i className="bi bi-clock w-4 h-4 text-muted-foreground inline-flex items-center justify-center leading-none" />
                          <span>{search}</span>
                        </button>
                        <button onClick={e => {
                  e.stopPropagation();
              ...
              {searchQuery.trim() && groupedSuggestions.aiIdentified.length > 0 && <div className="mb-4">
                  ...
                  <div className="flex flex-col gap-2">
                    {groupedSuggestions.aiIdentified.map((suggestion, idx) => (
                      ...
                      >
                        {suggestion.category === "People" ? <i className="bi bi-person w-4 h-4 inline-flex items-center justify-center leading-none" /> : suggestion.category === "Brand" ? <i className="bi bi-badge-tm" /> : <i className="bi bi-bounding-box-circles" />}
                  ...
                  <div className="flex flex-col gap-2">
                    {groupedSuggestions.otherTags.map((suggestion, idx) => {
                      ...
                      return (
                        ...
                        >
                          <i className="bi bi-tag w-4 h-4 inline-flex items-center justify-center leading-none" />
                          <span>{suggestion.value}</span>
                        </button>
                      );
                    })}
